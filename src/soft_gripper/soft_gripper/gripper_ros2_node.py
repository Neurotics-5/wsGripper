...
class GripperROS2Node(Node):
    def __init__(self, gripper_controller):
        ...
        self.command_sub = self.create_subscription(
            String, 'gripper/command', self.command_callback, 10, callback_group=sub_group
        )
        ...
        # Subscriptions in their own group
        self.left_sub = self.create_subscription(
            WrenchStamped, '/left/ndtact/force_estimate', self._left_force_cb, qos, callback_group=sub_group
        )
        self.right_sub = self.create_subscription(
            WrenchStamped, '/right/ndtact/force_estimate', self._right_force_cb, qos, callback_group=sub_group
        )


        # Timer for status + joint states
        self.status_timer = self.create_timer(0.01, self.status_callback, callback_group=timer_group)
